...
addGroupRouter.post("/", (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (userMap.has(uniqueId)) {
      if (isDm && uniqueId2) {
        ...
        groupMap.set(id, {
          groupName,
          users: [
            { userName: userMap.get(uniqueId)?.userName ?? "", id: uniqueId },
          ],
          chat: [],
          isDm,
        });
        const groups = Array.from(groupMap.entries()).map((value) => ({
          groupName: value[1].groupName,
          id: value[0],
          users: [{ userName: userMap.get(uniqueId)?.userName, id: uniqueId }],
        }));
